...
\title{实验一：基于 YOLOv8 与 Jetson 的桌面物体检测系统设计与验证}
...
\begin{abstract}
本文设计并实现一套面向桌面物体的端到端目标检测系统。系统以视频抽帧、样本筛选和 ISAT 人工标注为起点，依次完成类别映射、YOLO 检测格式转换、模型训练和 Jetson Orin 部署。数据集包含 595 张图像，按来源时间块划分为 421 张训练图像、87 张验证图像和 87 张测试图像；模型在验证集上的精确率、召回率、mAP@0.5 和 mAP@0.5:0.95 分别为 0.658、0.748、0.766 和 0.701。系统输出 mouse、laptop、cup、phone 四类目标，并在实时画面中叠加类别、边界框、置信度和帧率，同时通过 ROS2 发布 \code{/detections}。在完全独立、每类 20 张的 80 张实物照片上，系统通过 72 张，图片级准确率为 90\%；Jetson 演示视频显示约 15 FPS。报告给出独立测试判据、典型错误、部署参数、复现命令和 GitHub 分阶段提交证据。
\end{abstract}
...
\section{实验目标与系统概览}
本实验围绕“数据、模型、设备、接口”四个层面构建可运行、可复现、可验收的桌面物体检测链路。系统输入为 USB 摄像头的连续图像，核心模型为四类 YOLOv8n，输出包含类别、边界框和置信度，并在 Jetson 上实时显示。训练阶段负责从原始视频整理样本并获得模型权重，部署阶段将权重和推理参数迁移到边缘设备，接口阶段再把检测结果封装为 ROS2 消息。完整链路为：\markterm{摄像头采集} $\rightarrow$ \markterm{YOLOv8 检测} $\rightarrow$ \markterm{Jetson 实时显示} $\rightarrow$ \markterm{ROS2 发布}。

实验选择 mouse、laptop、cup 和 phone 四类常见桌面物体，在满足课程类别数量要求的同时，覆盖小目标、细长目标、反光表面和不同遮挡程度等情况。最终验收包含三个层面：带标注数据上的模型指标用于判断训练质量；独立实物照片用于衡量跨拍摄条件的识别可靠性；Jetson 视频和程序文件用于说明模型已经进入实时应用链路。
...
\FloatBarrier
\section*{结论}
本实验完成了从数据采集、标注、YOLOv8 训练到 Jetson 实时检测和 ROS2 发布的完整链路。四类独立实物照片通过率为 90\%，Jetson 视频显示约 15 FPS，达到本实验的主要验收指标。论文式报告、源文件、数据、模型、程序、视频、运行说明和错误案例均已整理并上传 GitHub，为后续现场复现和功能扩展提供了完整基础。
